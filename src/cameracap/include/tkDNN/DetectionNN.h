...
class DetectionNN {
    ...
    public:
         ...
         */
        void draw(std::vector<cv::Mat>& frames, std::vector<int> &detret, std::vector<std::string> &classesnames) {
            tk::dnn::box b;
            int x0, w, x1, y0, h, y1;
            int objClass;
            ...
            for(int bi=0; bi<frames.size(); ++bi){
                ...
                for(int i=0; i<batchDetected[bi].size(); i++) { 
                    ...
                    detret.push_back(b.x);
                    detret.push_back(b.y);
                    detret.push_back(b.w);
                    detret.push_back(b.h);
